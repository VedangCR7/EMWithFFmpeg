...
router.post('/profile', [
  body('businessName').isLength({ min: 2 }).withMessage('Business name must be at least 2 characters'),
  ...
  body('businessAddress').optional().isLength({ min: 10 }).withMessage('Business address must be at least 10 characters'),
  body('businessDescription').optional().isLength({ min: 20 }).withMessage('Business description must be at least 20 characters'),
  body('selectedBusinessCategoryId').notEmpty().withMessage('Business category is required'),
  ...
  try {
    ...
    // Create business profile (this would typically be for a customer)
    // For now, we'll create a mock response since we don't have customer context
