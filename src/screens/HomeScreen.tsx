...
const GreetingCard: React.FC<GreetingCardProps> = React.memo(({ item, cardWidth, theme, categoryTemplates, searchQuery, navigation, onCardPress }) => {
  ...
  const handlePress = useCallback(() => {
    if (!item || !item.thumbnail) {
      if (__DEV__) {
        devError('G•Ó [GREETING CARD] Invalid item:', item);
      }
      return;
    }
...
const HomeScreen: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    const unsubscribe = authService.onAuthStateChanged(user => {
      console.log('=Éˆ‰ [HOMESCREEN] Auth state changed, new user:', user?.id || 'null');
      setUserProfile(user);
      
      // Clear business profile selection when user changes
      if (!user) {
        console.log('=É∫¶ [HOMESCREEN] User logged out, clearing business profile selection');
  ...
  useEffect(() => {
    ...
    const loadBusinessProfiles = async () => {
      ...
      console.log('=ÉÙÔ [HOMESCREEN] Loading business profiles for user:', currentUserId);
  ...
  useFocusEffect(
    useCallback(() => {
      console.log('=Éˆ‰ [HOMESCREEN] Screen focused - refreshing business profiles...');
      const currentUserId = userProfile?.id || authService.getCurrentUser()?.id;
      if (currentUserId) {
  ...
  useEffect(() => {
    ...
    console.log('=ÉˆÏ [HOMESCREEN DEBUG] Storing profile data:', {
  ...
  // Loads images in batches: Critical GÂ∆ High GÂ∆ Medium GÂ∆ Low priority
      ...
      try {
        ...
        const immediateApiPromises = [
          requestDeduplication.deduplicate(
            ...
            if (__DEV__) {
              console.error('[FEATURED CONTENT] G•Ó Error loading:', err?.message || err);
          ...
          // }).catch(err => {
          //   if (__DEV__) {
          //     console.error('[VIDEO CONTENT] G•Ó Error loading:', err?.message || err);
    ...
    const loadBusinessCategories = async () => {
      ...
      try {
        ...
        if (__DEV__ && response && response.success) {
          ...
          console.log(`=ÉÙÔ [HOME BUSINESS CATEGORIES] Loaded ${categories.length} categories`);
        }
        if (isMounted && response && response.success) {
          ...
          console.log('=ÉˆÏ [BUSINESS CATEGORY FILTER]', {
          ...
          const filteredCategories = allCategories.filter((category: BusinessCategory) => {
            ...
            console.log('=ÉˆÏ [FILTERING CATEGORY]', {
  ...
  const handleSearch = useCallback(async () => {
    ...
    // Debug logging
    console.log('=ÉˆÏ [SEARCH DEBUG] Search query:', searchQuery);
    console.log('=ÉˆÏ [SEARCH DEBUG] Business categories available:', businessCategories.map(c => c.name));
    console.log('=ÉˆÏ [SEARCH DEBUG] Matching business categories:', matchingBusinessCategoryNames);
    console.log('=ÉˆÏ [SEARCH DEBUG] Matching greeting categories:', matchingCategoryNames);
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        console.log('=ÉˆÏ [SEARCH DEBUG] General category results count:', generalCategoryResults.length);
        console.log('=ÉˆÏ [SEARCH DEBUG] Business category results count:', businessCategoryResults.length);
        ...
        console.log('=ÉˆÏ [SEARCH DEBUG] Total results before dedup:', allResults.length);
        console.log('=ÉˆÏ [SEARCH DEBUG] Unique results after dedup:', uniqueResults.length);
        console.log('=ÉˆÏ [SEARCH DEBUG] Business templates in results:', convertedBusinessCategoryResults.length);
...
const handleTemplatePress = useCallback((template: Template | VideoContent | any) => {
  ...
  const handleBusinessCategoryPress = useCallback(async (category: BusinessCategory) => {
    console.log('=ÉˆÏ [BUSINESS CATEGORY PRESS]', {
           ...
           >
             <View style={styles.modalContent}>
               <TouchableOpacity 
                 ...
                 onPress={closeModal}
               >
                 <Text style={styles.closeButtonText}>G£Ú</Text>
               </TouchableOpacity>
               {selectedTemplate && (
                 <>
              ...
              >
                <View style={styles.upcomingEventsModalHeader}>
                  ...
                  <TouchableOpacity 
                    ...
                    activeOpacity={0.7}
                  >
                    <Text style={[styles.upcomingEventsCloseButtonText, { color: theme.colors.text }]}>G£Ú</Text>
                  </TouchableOpacity>
                </View>
              </LinearGradient>
            ...
            <View style={styles.upcomingEventsModalContent}>
              ...
              >
                <View style={styles.upcomingEventsModalHeader}>
                  ...
                  >
                    <Text style={styles.upcomingEventsCloseButtonText}>G£Ú</Text>
                  </TouchableOpacity>
                </View>
              </LinearGradient>
            ...
            >
              <View style={styles.customerSupportModalContent}>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <Text style={styles.customerSupportCloseButtonText}>G£Ú</Text>
                </TouchableOpacity>
