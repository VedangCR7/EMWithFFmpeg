...
class GreetingTemplatesService {
  ...
  // Download a template
  async downloadTemplate(templateId: string, businessProfileId?: string): Promise<boolean> {
    try {
      const payload: any = {};
      ...
      const response = await api.post(`/api/mobile/greetings/templates/${templateId}/download`, payload);
