    ...
    try {
      ...
      if (userId) {
        try {
          ...
          // Filter for both TEMPLATE type and calendar/festival categories
          const posterDownloads = backendDownloads.downloads.filter(d => {
            // Include TEMPLATE types (regular posters)
            if (d.resourceType === 'TEMPLATE') {
              return true;
            }
            // Include calendar/festival items regardless of resourceType
            const category = (d.category || '').toLowerCase();
            return category === 'calendar' || category === 'festival';
          });
          
          // Map backend downloads to DownloadedPoster format
          const mappedPosters = posterDownloads.map(download => ({
            id: download.id,
          ...
          console.log('🔍 [DEBUG] Backend downloads breakdown:', {
            totalDownloads: backendDownloads.downloads.length,
            templateDownloads: backendDownloads.downloads.filter(d => d.resourceType === 'TEMPLATE').length,
            calendarCategoryDownloads: backendDownloads.downloads.filter(d => 
              (d.category || '').toLowerCase() === 'calendar' || 
              (d.category || '').toLowerCase() === 'festival'
            ).length,
            filteredPosters: posterDownloads.length
          });
