...
class TemplatesBannersApiService {
  ...
  // Download template
  async downloadTemplate(id: string, businessProfileId?: string): Promise<{ success: boolean; message: string }> {
    try {
      const payload: any = {};
      ...
      const response = await api.post(`/api/mobile/templates/${id}/download`, payload);
      return response.data;
    } catch (error) {
