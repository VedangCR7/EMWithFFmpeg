...
class BusinessCategoryPostersApiService {
   ...
   * Download a poster
   */
  async downloadPoster(posterId: string, businessProfileId?: string): Promise<{ success: boolean; message: string; downloadUrl?: string }> {
    try {
      ...
      const downloadResponse = await api.post('/api/mobile/downloads/track', {
        ...
        businessProfileId: businessProfileId || undefined // Include business profile ID if available
      });
