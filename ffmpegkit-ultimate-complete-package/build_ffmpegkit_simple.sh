...
echo "=========================================="
echo "FFmpegKit Build Script (Simplified)"
echo "=========================================="
echo ""
...
build_ffmpegkit() {
    ...
    # Note: Excluding libiconv due to groff dependency issue
    if ./android.sh \
        --lts \
        --enable-gpl \
        --enable-freetype \
...
main() {
    ...
    local essential_tools=("git" "wget" "unzip" "python3" "cmake" "autoconf" "automake" "yasm" "nasm")
    for tool in "${essential_tools[@]}"; do
        if ! command_exists "$tool"; then
            print_error "Essential tool '$tool' not found. Please install it manually."
            exit 1
        fi
    done
